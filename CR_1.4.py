...
class Explosion:
    def __init__(self, x: int, y: int, color: Tuple[int, int, int]):
        ...
        self.particles = []
        self.completed = False  # Track if animation is complete
        for _ in range(EXPLOSION_PARTICLES):
            ...
            self.particles.append({
                ...
                'final_x': 0,  # Store final position
                'final_y': 0
            })
    ...
    def draw(self, window):
        ...
        if progress >= 1 and not self.completed:
            # Store final positions when animation completes
            for particle in self.particles:
                particle['final_x'] = self.x + particle['dx'] * CELL_SIZE
                particle['final_y'] = self.y + particle['dy'] * CELL_SIZE
            self.completed = True
        ...
        if not self.completed:
            ...
            for particle in self.particles:
                ...
                size = particle['size'] * (1 - progress * 0.5)  # Reduce size shrinking
                pygame.draw.circle(window, self.color, 
                                 (int(particle_x), int(particle_y)), 
                                 int(size))
        else:
            # Draw particles in their final positions
            for particle in self.particles:
                pygame.draw.circle(window, self.color, 
                                 (int(particle['final_x']), int(particle['final_y'])), 
                                 int(particle['size'] * 0.5))  # Fixed final size
        
        return False  # Never return True so explosions are never removed
