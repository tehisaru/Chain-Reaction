...
class Game:
    ...
    def check_winner(self) -> bool:
        ...
        return len(colors) == 1 and len(self.get_all_dots()) > 0
...
def update_game(game, moving_blobs):
    current_time = time.time()
    completed_blobs = []
    cells_to_check = set()  # Store cells that need to be checked for chain reaction
    ...
    for blob in moving_blobs:
        if blob.update_position(current_time):
            ...
            # Add dot to cell and check if it needs chain reaction
...
def make_move(game, row, col, moving_blobs):
    ...
    # Only change turns if there are no animations in progress
    if not moving_blobs:
        game.turns_played += 1
